...
if (strlen($_SESSION['aid'] == 0)) {
    ...
    if (isset($_GET['pid'])) {
        ...
        if (empty($decoded_pid) || !is_numeric($decoded_pid)) {
            ...
            // Query to delete the herd manager from the database
            $query = mysqli_query($con, "DELETE FROM tblherd_managers WHERE id='$decoded_pid'");

            if ($query) {
                echo "<script>alert('Herd manager deleted successfully.');</script>";
                echo "<script>window.location.href='manage-herd-managers.php'</script>";
            } else {
                echo "<script>alert('Error deleting herd manager.');</script>";
                echo "<script>window.location.href='manage-herd-managers.php'</script>";
            }
        }
    } else {
